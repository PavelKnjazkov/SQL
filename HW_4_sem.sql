-- Cоздаем и наполняем таблицу использую sql файл с 4-го семинара (lesson_4.sql)
USE lesson_4
...
SELECT from_user_id AS 'id отправителя',
    ...
    FROM users
    WHERE id = messages.from_user_id) AS 'ФИО отправителя', COUNT(*) AS `Отправлено сообщений`
FROM messages
