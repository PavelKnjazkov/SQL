...
указав имя и фамилию пользователя, количество отправленных сообщений и место в рейтинге (первое место 
у пользователя с максимальным количеством сообщений) . (используйте DENSE_RANK)*/

SELECT 
	DENSE_RANK() OVER (ORDER BY COUNT(from_user_id) DESC) AS 'Позиция в рейтинге',
	COUNT(from_user_id) AS 'Кол-во отправленных сообщений',
	CONCAT(firstname, ' ', lastname) AS 'Юзеры'
FROM users u
